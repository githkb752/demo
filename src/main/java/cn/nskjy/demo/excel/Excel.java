...
public class Excel {
    public static void main(String[] args) {
        try {
            ...
            //获取指定列的值
            readSpecifyColumns(new File("C:\\Users\\Administrator\\Desktop\\测试表.xls"));

            //获取指定行的值
//            readSpecifyRows(new File("C:\\Users\\Administrator\\Desktop\\测试表.xls"));

            //读取行列的值
//            readRowsAndColums(new File("C:\\Users\\Administrator\\Desktop\\测试表.xls"));

            //将获取到的值写入到TXT或者xls中
//            copy_excel(new File("C:\\Users\\Administrator\\Desktop\\测试表.xls"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void readSpecifyColumns(File file)throws Exception{
        ...
        for (int i = 1; i < rsRows; i++) {
            ...
            Cell cell_value = readsheet.getCell(4, i);  //第三列的值，此处需要手动更改，获取不同列的值
    ...
    private static void readRowsAndColums(File file) throws BiffException, IOException {
        //1:创建workbook
        Workbook workbook=Workbook.getWorkbook(new File("C:\\Users\\Administrator\\Desktop\\测试表.xls"));
        //2:获取第一个工作表sheet
